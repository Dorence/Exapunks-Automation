class Stack:
    numb_lookup = {"0": None, "9": "0", "8": "9", "7": "8", "6": "7"}
    value_lookup = {
        "0": 10,
        "9": 9,
        "8": 8,
    ...
    def get_combine(from_stack, to_stack):
        if from_stack.is_faces and to_stack.is_faces:
            ...
            if suit_match:
                new_stack = Stack(
                    to_stack.back, from_stack.front, to_stack.length + from_stack.length
                )
                return new_stack
            ...
            value_match = (
                Stack.numb_lookup[from_stack.back.value] == to_stack.front.value
            )
